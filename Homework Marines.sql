...
-- Para ver la ruta de origen donde poner los csv.
SELECT @@global.secure_file_priv;

/*Importacion de las tablas*/

#Eliminar la tabla por si existe
DROP TABLE IF EXISTS `gasto`;

-- Crear nueva tabla gastos --
CREATE TABLE gasto (
  IdGasto INTEGER,
  IdSucursal INTEGER,
  IdTipoGasto INTEGER,
...
-- Cargando datos a la tabla de gastos --
...
SET 
    ...
    Monto = CAST(REGEXP_REPLACE(@Monto, '[^0-9.]', '') AS DECIMAL(12,2));

-- Ver que la tabla se cargo bien --
SELECT * FROM gasto;

-- Eliminar la tabla por si existe --
DROP TABLE IF EXISTS `compra`;

-- Crear nueva tabla compra --
CREATE TABLE compra (
  IdCompra INTEGER,
  Fecha DATE,
  IdProducto INTEGER,
...
-- Cargando datos a la tabla de compra --
...
SET 
    ...
    Precio = CAST(REGEXP_REPLACE(@Precio, '[^0-9.]', '') AS DECIMAL(12,2)),
    IdProveedor = CAST(REGEXP_REPLACE(@IdProveedor, '[^0-9]', '') AS UNSIGNED);
...
-- Cargando datos a la tabla de venta --
...
SET 
    Fecha = STR_TO_DATE(@Fecha, '%Y-%m-%d'),
    Fecha_Entrega = STR_TO_DATE(@Fecha_Entrega, '%Y-%m-%d'),
    Precio = IF(@Precio <> '', CAST(REGEXP_REPLACE(@Precio, '[^0-9.]', '') AS DECIMAL(12,2)), 0),
    Cantidad = IF(@Cantidad REGEXP '^[0-9]+$' AND @Cantidad <> '', CAST(@Cantidad AS UNSIGNED), 0);
...
-- Cargando datos a la tabla de sucursal --
...
-- TRUNCATE TABLE sucursal; Se usa para para eliminar todos los registros de una tabla en una base de datos, pero mantiene la estructura de la tabla para futuros usos

-- Eliminar la tabla por si existe --
DROP TABLE IF EXISTS cliente;

-- Crear nueva tabla cliente --
...
-- Cargando datos a la tabla de cliente --
...
-- Cargando datos a la tabla de tipo_gasto --
...
SET Monto_Aproximado = CAST(@Monto_Aproximado AS DECIMAL(10,2));
...
-- Cargando datos a la tabla de proveedor --
...
-- Cargando datos a la tabla de empleado --
...
/*Tipos de Datos*/
ALTER TABLE `cliente`
	ADD COLUMN IF NOT EXISTS `Latitud` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Y`,
	ADD COLUMN IF NOT EXISTS `Longitud` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Latitud`;
...
-- -- Solo si 'Salario' no existe, entonces agrégalo
...
UPDATE empleado
SET Salario2 = REGEXP_REPLACE(Salario2, '[^0-9.]', ''),
    Salario = ROUND(CAST(REGEXP_REPLACE(Salario2, '[^0-9.]', '') AS DECIMAL(10, 2)), 2)
WHERE IdEmpleado IS NOT NULL;


UPDATE empleado SET Salario = Salario2 WHERE IdEmpleado IS NOT NULL;
UPDATE empleado SET Salario = Salario2 WHERE IDEmpleado IS NOT NULL;


SELECT IDEmpleado, Salario2 FROM empleado WHERE Salario2 REGEXP '[^0-9.]';

UPDATE empleado SET Salario = Salario2 WHERE IDEmpleado IS NOT NULL;
ALTER TABLE empleado MODIFY COLUMN Salario2 DECIMAL(10,2);
UPDATE empleado SET Salario2 = TRIM(Salario2);

SHOW COLUMNS FROM empleado LIKE 'Salario';

UPDATE `empleado` SET Salario = Salario2;
ALTER TABLE `empleado` DROP `Salario2`;

-- Vuelve a activar el modo de actualización segura
#SET SQL_SAFE_UPDATES = 1;
...
CREATE TABLE IF NOT EXISTS `localidad` (
  ...
  `IdProvincia` int(11) NOT NULL,
   `Latitud` DOUBLE NOT NULL DEFAULT '0', -- Agrega las columnas Latitud y Longitud
  `Longitud` DOUBLE NOT NULL DEFAULT '0',
  PRIMARY KEY (`IdLocalidad`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;


-- Actualización de la columna Longitud en la tabla localidad **********************************ERROR**************************************
UPDATE localidad l 
...
-- TP INTEGRADOR 
-- 1) Clientes que hicieron compras en 2020 y también en 2019.
SELECT DISTINCT c.IdCliente, c.Nombre_y_Apellido
FROM dim_cliente c
JOIN fact_venta v2020 ON c.IdCliente = v2020.IdCliente
JOIN fact_venta v2019 ON c.IdCliente = v2019.IdCliente
WHERE YEAR(v2020.Fecha) = 2020 AND YEAR(v2019.Fecha) = 2019

-- Crear índices necesarios para mejorar el rendimiento
-- Índices para la tabla dim_cliente
CREATE INDEX idx_dim_cliente_IdCliente ON dim_cliente (IdCliente);

-- Índices para la tabla fact_venta
CREATE INDEX idx_fact_venta_IdCliente ON fact_venta (IdCliente);
CREATE INDEX idx_fact_venta_Fecha ON fact_venta (Fecha);

-- 2) Mostrar el porcentaje de Ventas que representa cada Mes, dentro del total del Año, sólo para 2018.

-- índice para mejorar el rendimiento de la consulta
ALTER TABLE `fact_venta` ADD INDEX(`Fecha`);

-- Consulta para obtener el porcentaje de Ventas por Mes en 2018
SELECT 
    MONTH(fv.Fecha) AS Mes,
    COUNT(*) AS Ventas,
    COUNT(*) / total.TotalVentas * 100 AS PorcentajeVentas
FROM 
    fact_venta fv
JOIN
    (SELECT COUNT(*) AS TotalVentas FROM fact_venta WHERE YEAR(Fecha) = 2018) AS total
ON
    YEAR(fv.Fecha) = 2018
GROUP BY 
    Mes
